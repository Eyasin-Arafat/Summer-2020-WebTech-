			...
			$remember = [];
				...
				if (isset($_POST["submit"])) 
				{
					if (isset($_POST["uname"])) 
					{
						...
						if ($uname == '') 
						{
							$unameErr = "User Name can not be null";
						}
					}			
					else 
					{
				...
				if (isset($_POST["pass"])) 
				{
					...
					if ($pass == '') 
					{
						$passErr = 'Password can not be null';
					}
				} 
				else 
				{
				...
				if (isset($uname) && isset($pass)) 
				{
					if (isset($_COOKIE['uname']) && isset($_COOKIE['pass'])) 
					{
						if (strtolower($uname) == $_COOKIE['uname'] && $pass == $_COOKIE['pass']) 
						{
							...
							{
								setcookie('remember', $uname, time() + (20 * 365 * 25 * 70 * 70));
							}		 
							else 
							{
								setcookie('remember', "");
                        ...
                        <table>
                            ...
                            <tr>
                                <td colspan="2" style="border-top: 1px solid black; padding-top: 10px;">
									...
									<?php 
									if (isset($remember) && in_array('yes', $remember)) echo "checked"; 
									?>
